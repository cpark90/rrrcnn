...
class _AttentionLayer(Function):
    @staticmethod
    def forward(
        ctx,
        input,
        ...
        output = input.new_empty(
            _AttentionLayer._output_size(input, ctx.grid_size)
        )

        if not input.is_cuda:
            raise NotImplementedError
        else:
            cur_im2col_step = _AttentionLayer._cal_im2col_step(input.shape[0], ctx.im2col_step)
            assert (input.shape[0] % cur_im2col_step) == 0, "im2col step must divide batchsize"

            _C.deformable_by_grad_forward(
                input,
                offset,
                output,
                ctx.grid_size[1],
    ...
    def backward(ctx, grad_output):
        ...
        if not grad_output.is_cuda:
            ...
            if ctx.needs_input_grad[0] or ctx.needs_input_grad[1]:
                ...
                _C.deformable_by_grad_backward_input(
                    input,
                    offset,
                    grad_output,
                    grad_input,
...
deformable_by_grad_layer = _AttentionLayer.apply

class DeformByGradLayer(nn.Module):
    def __init__(
        self,
        in_channels,
        shift=0,
        ...
        """
        super(DeformByGradLayer, self).__init__()
    ...
    def forward(self, x, offset, grid_size):
        if x.numel() == 0:
