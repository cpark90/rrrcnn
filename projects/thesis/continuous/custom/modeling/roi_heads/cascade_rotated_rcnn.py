...
from .rotated_fast_rcnn import CustomizedRROIHeads, RotatedFastRCNNOutputLayers


@ROI_HEADS_REGISTRY.register()
class CascadeRROIHeads(CustomizedRROIHeads):
    """
        ...
        for match_iou, bbox_reg_weights in zip(cascade_ious, cascade_bbox_reg_weights):
            ...
            box_predictors.append(
                RotatedFastRCNNOutputLayers(
                    ...
                    box2box_transform=Box2BoxTransformRotated(weights=bbox_reg_weights),
                )
            )
    ...
    def _run_stage(self, features, proposals, stage):
        ...
        if not torch.isfinite(box_features).all():
            print("cascade")
            print(box_features[~torch.isfinite(box_features)])
