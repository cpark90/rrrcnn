...
from projects.Pretraining.imagenet_pretrain.continuous.group_equivariant_conv.group_equivariant_conv import *
from .rearr_conv import RearrConv
from .rearrnet import RearrNet

__all__ = ["RearrBottleneckBlock", "build_rearrnet_backbone_pretrain"]
...
class RearrBottleneckBlock(ResNetBlockBase):
    def __init__(
        self,
        in_channels,
        out_channels,
    ...
    def forward(self, x):
        ...
        offset = F.leaky_relu_(self.conv2_offset_a(out))
        offset_rate = torch.tanh(self.conv2_offset_b(out)) * self.L
        #
        offset = torch.mul(offset, offset_rate)
...
def build_rearrnet_backbone_pretrain(cfg, input_channels, num_classes):
    """
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        if deform_on_per_stage[idx]:
            stage_kargs["block_class"] = RearrBottleneckBlock
