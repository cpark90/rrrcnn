...
from detectron2.modeling.backbone.resnet import BasicBlock, make_stage, ResNet
...
__all__ = [
    "build_resnet_pretrain_backbone",
]
...
class BottleneckBlock(CNNBlockBase):
    ...
    def forward(self, x):
        grid_size = (int(x.size(2) / self.stride), int(x.size(3) / self.stride))
        offset = torch.randn(x.size(0), x.size(1) * 2, grid_size[0], grid_size[1], device=x.device) * self.noise_var
        out = self.conv1(self.defem_layer(x, offset, grid_size, 0.5 if self.stride == 2 else 0))
        out = F.relu_(out)
        ...
        if self.shortcut is not None:
            grid_size = (int(x.size(2) / self.stride), int(x.size(3) / self.stride))
            offset = torch.randn(x.size(0), x.size(1) * 2, grid_size[0], grid_size[1], device=x.device) * self.noise_var
            shortcut = self.shortcut(self.defem_layer(x, offset, grid_size, 0.5 if self.stride == 2 else 0))
        else:
            shortcut = x
...
class BasicPStem(CNNBlockBase):
    """
    The standard ResNet stem (layers before the first residual block).
    """

    def __init__(self, in_channels=3, out_channels=64, noise_var=0.0, norm="BN"):
        """
        Args:
...
def build_presnet_backbone(cfg, input_shape, num_classes):
    """
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        blocks = make_pstage(**stage_kargs)
        in_channels = out_channels
        out_channels *= 2
...
def build_resnet_pretrain_backbone(cfg, input_shape, num_classes):
    """
    Create a ResNet instance from config.
