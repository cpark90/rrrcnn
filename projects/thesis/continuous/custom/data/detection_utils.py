...
def transform_instance_annotations_rotated(
    ...
    """
    Apply transforms to rotated box annotations of a single instance.
...
def gen_crop_transform_with_instance(crop_size, image_size, instance):
    ...
    bbox = BoxMode.convert(instance["bbox"], instance["bbox_mode"], BoxMode.XYXY_ABS)[0]
...
def build_test_transform_gen(cfg, is_train):
    """
    Create a list of :class:`TransformGen` from config.
    Now it includes resizing and flipping.
    ...
    logger = logging.getLogger(__name__)
    tfm_gens = []
    tfm_gens.append(T.Normal())
    if is_train:
        logger.info("TransformGens used in training: " + str(tfm_gens))
    return tfm_gens

def build_test_512_transform_gen(cfg, is_train):
    """
    Create a list of :class:`TransformGen` from config.
    Now it includes resizing and flipping.
    ...
    logger = logging.getLogger(__name__)
    tfm_gens = []
    tfm_gens.append(T.Resize(700))
    if is_train:
        logger.info("TransformGens used in training: " + str(tfm_gens))
    return tfm_gens

def build_test_768_transform_gen(cfg, is_train):
    """
    Create a list of :class:`TransformGen` from config.
    Now it includes resizing and flipping.
    ...
    logger = logging.getLogger(__name__)
    tfm_gens = []
    tfm_gens.append(T.Resize(768))
    if is_train:
        logger.info("TransformGens used in training: " + str(tfm_gens))
    return tfm_gens

def build_test_1024_transform_gen(cfg, is_train):
    """
    Create a list of :class:`TransformGen` from config.
    Now it includes resizing and flipping.
