...
class RotateTransform(Transform):
    ...
    def __init__(self, degree, w, h, resample=False, expand=False, center=None, fill=0):
        if center is None:
            (cX, cY) = (w // 2, h // 2)
        else:
            cX, cY = center
    ...
    def apply_image(self, img:np.ndarray):
        ...
        M = cv2.getRotationMatrix2D((self.cX, self.cY), self.degree, 1.0)
        cos = np.abs(M[0, 0])
        sin = np.abs(M[0, 1])

        # compute the new bounding dimensions of the image
        nW = int((self.h * sin) + (self.w * cos))
        nH = int((self.h * cos) + (self.w * sin))
...
def NoOp_rotated_box(transform, rotated_boxes):
    coords = np.asarray(rotated_boxes).reshape(-1, 5)
    return coords
...
def VFlip_rotated_box(transform, rotated_boxes):
    ...
    # Transform angle
    rotated_boxes[:, 4] = 180-rotated_boxes[:, 4]
    return rotated_boxes
