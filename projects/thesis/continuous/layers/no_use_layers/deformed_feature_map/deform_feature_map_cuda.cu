...
void shape_check_defe_map(
  ...
  TORCH_CHECK(
      (nOffsetPlane * group == 2 * nInputPlane),
...
int deform_feature_map_backward_input_cuda(
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    deform_feature_map_col2im_coord(
        gradOutput[elt],
        input[elt],
        offset[elt],
        nInputPlane,
