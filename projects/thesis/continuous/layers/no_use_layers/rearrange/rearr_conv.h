...
inline int rearr_conv_forward(
  ...
  if (input.type().is_cuda()) {
    ...
    return rearr_conv_forward_cuda(
        input,
        weight,
        offset,
        output,
...
inline int rearr_conv_backward_input(
    at::Tensor input,
    at::Tensor offset,
    at::Tensor gradOutput,
    at::Tensor gradInput,
  ...
  if (gradOutput.type().is_cuda()) {
    ...
    TORCH_CHECK(weight.type().is_cuda(), "weight tensor is not on GPU!");
    TORCH_CHECK(offset.type().is_cuda(), "offset tensor is not on GPU!");
    return rearr_conv_backward_input_cuda(
        input,
        offset,
        gradOutput,
        gradInput,
...
inline int rearr_conv_backward_filter(
    at::Tensor input,
    at::Tensor offset,
    at::Tensor gradOutput,
    at::Tensor gradWeight, // at::Tensor gradBias,
  ...
  if (gradOutput.type().is_cuda()) {
    ...
    TORCH_CHECK(input.type().is_cuda(), "input tensor is not on GPU!");
    TORCH_CHECK(offset.type().is_cuda(), "offset tensor is not on GPU!");
    return rearr_conv_backward_parameters_cuda(
        input,
        offset,
        gradOutput,
        gradWeight,
