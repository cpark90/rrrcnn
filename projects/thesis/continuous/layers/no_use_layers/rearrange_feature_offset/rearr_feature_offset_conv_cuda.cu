...
#include "rearr_feature_offset_conv.h"
...
void shape_check_rearr_feature_offset(
    at::Tensor input,
    at::Tensor offset,
  ...
  long outputHeight =
      (gH - (dilationH * (kH - 1) + 1)) / dH + 1;
  long outputWidth =
      (gW - (dilationW * (kW - 1) + 1)) / dW + 1;
...
int rearr_feature_offset_conv_forward_cuda(
  ...
  shape_check_rearr_feature_offset(
      input,
      offset,
      NULL,
      weight,
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    rearr_feature_offset_conv_im2col(
        input[elt],
        offset[elt],
        nInputPlane,
        inputHeight,
...
int rearr_feature_offset_conv_backward_input_cuda(
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    ...
    rearr_feature_offset_conv_col2im(
        columns,
        offset[elt],
        nInputPlane,
        inputHeight,
...
int rearr_feature_offset_conv_backward_parameters_cuda(
  ...
  if (input.ndimension() == 3) {
    ...
    offset = offset.view(
        {1, offset.size(0), offset.size(1), offset.size(2)});
    gradOutput = gradOutput.view(
        {1, gradOutput.size(0), gradOutput.size(1), gradOutput.size(2)});
  }
